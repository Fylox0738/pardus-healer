...
| `app.py` | 558 | Ana pencere kabuğu | En büyük dosya; sidebar, sayfa geçişleri, genel pencere durumu — bkz. TECHNICAL_AUDIT (tek dosyada çok sorumluluk riski) |
...
| `fix_runner.py` | — (yardımcı) | Onarım komutu çalıştırma | `card.py` ve `app.py`'nin iki worker'ı arasında paylaşılan ortak `run_fix_command()` — eskiden üç yerde kopyalıydı (bkz. TECHNICAL_AUDIT.md / FIX_DECISION_MATRIX.md P2 madde 21) |
...
| `pulse_dialog.py` | 101 | "Pardus Nabız" diyaloğu | `core/pulse.py` üzerinden birikmiş SOS olaylarının toplu görünümü — **demo veri riski için TECHNICAL_AUDIT'e bakın** |
